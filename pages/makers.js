...
const makerslist = () => {
  return (
    <Layout
      title={`${koteibun.date}${koteibun.next}で見れるAVメーカー一覧`}
      description={`${koteibun.date}${koteibun.next}で見れるAVメーカー作品まとめ。アダルトビデオは見放題？見たいAVが見れるか見れないか知りたいときにご利用ください。`}
      keyword={koteibun.keyword}
      image={""}
      url={`${koteibun.domain}/makers`}
      type={"article"}
    >
      <div>
        <h1 className="sm:text-2xl sm:font-mono font-serif text-xl py-4 pt-8">{`${koteibun.date}${koteibun.next}で見れるAVメーカー一覧`}</h1>
      </div>
      <div className="sm:px-4 py-4 font-sans">
        <p>{koteibun.teikei}</p>
      </div>
      {
        <div className="font-sans mt-10">{`メーカー数：${makers.length}件`}</div>
      }
