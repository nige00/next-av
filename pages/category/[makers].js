...
export default function Post({ post }) {
  ...
  // URLに合わせて女優の出演作品を抽出
  let makermei = posts.filter((item) => {
    if (item.meka == post.makerName) {
      return item;
    } else if (item.meka.length > 1) {
      for (let i = 0; i < item.meka.length; i++) {
        if (item.meka[i] == post.makerName) {
          return item;
        }
      }
    }
  ...
  const newlist = [];
  if (makermei.length <= 10) {
    for (let i = 0; i < makermei.length; i++) {
      newlist.push(makermei[i]);
    }
  } else {
    for (let i = 0; i < 10; i++) {
      newlist.push(makermei[i]);
  ...
  return (
    <Layout
      title={`${koteibun.date}${koteibun.next}で見れる『${post.makerName}』作品の配信状況一覧`}
      description={`${koteibun.date}${koteibun.next}で見れる${post.makerName}作品まとめ。アダルトビデオは見放題？見たいAVが見れるか見れないか知りたいときにご利用ください。`}
      keyword={koteibun.keyword}
      image={makermei[0].avImg}
      url={`${koteibun.domain}/category/${post.pageUrl}`}
      type={"article"}
    >
      <div>
        <h1 className="sm:text-2xl sm:font-mono font-serif text-xl py-4 pt-8">{`${koteibun.date}${koteibun.next}で見れる『${post.makerName}』作品一覧`}</h1>
      </div>
...
export async function getStaticPaths() {
  const paths = makers.map((post) => {
    return {
      params: {
        makers: String(post.pageUrl),
      },
    };
  });
...
export async function getStaticProps({ params }) {
  const post = makers.find(function (post) {
    return post.pageUrl === params.makers;
  });
